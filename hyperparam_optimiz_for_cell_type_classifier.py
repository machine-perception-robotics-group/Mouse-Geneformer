...
validset = trainset_v4.select([i for i in range(round(len(trainset_v4)*0.8),len(trainset_v4))])
...
print("classifier_trainset['label']: {}".format(np.unique(classifier_trainset["label"])))
print("classifier_validset: {}".format(classifier_validset))
print("classifier_validset['label']: {}".format(np.unique(classifier_validset["label"])))
...
def model_init():
    ...
    model = BertForSequenceClassification.from_pretrained("/path/to/pretrained/model/{}/models/".format(pretrain_model), 
